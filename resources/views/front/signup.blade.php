    ...
    <section class="signup-sec">
        ...
        <div class="container">
            ...
            <form action="{{ URL::to('/signup/signup') }}" method="post">
                ...
                <input type="text" name="name" placeholder="Họ tên" class="w-100" value="{{Session::get('name')}}">
                <input type="text" name="email" placeholder="Email" class="w-50" value="{{ Session::get('email') }}" readonly="">
                <input type="text" name="mobile" placeholder="Số điện thoại" class="w-50" value="{{ old('mobile') }}">
                <input type="text" name="referral_code" placeholder="Mã giới thiệu (Tùy chọn)" class="w-100" value="{{ Request()->referral_code }}">
                @else
                <input type="text" name="name" placeholder="Họ tên" class="w-100" value="{{ old('name') }}">
                <input type="text" name="email" placeholder="Email" class="w-50" value="{{ old('email') }}">
                <input type="number" name="mobile" placeholder="Số điện thoại" class="w-50" value="{{ old('mobile') }}">
                <input type="password" name="password" placeholder="Mật khẩu" class="w-50">
                <input type="password" name="password_confirmation" placeholder="Xác nhận mật khẩu" class="w-50">
                <input type="text" name="referral_code" placeholder="Mã giới thiệu (Tùy chọn)" class="w-100" value="{{ Request()->referral_code }}">
                @endif
                <label class="accept-check w-100" for="accept">
                    <input type="checkbox" name="accept" id="accept" required="">
                    <p class="already-p">Bạn đồng ý các<a href="{{URL::to('/terms')}}"> Điều khoản & dịch vụ </a></p>
                </label>
                <button class="btn">Đăng ký</button>
                <p class="already">Bạn đã có tài khoản?<a href="{{URL::to('/signin')}}"> Đăng nhập</a></p>
            </form>
        </div>
    </section>

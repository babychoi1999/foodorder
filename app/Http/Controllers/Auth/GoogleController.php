...
class GoogleController extends Controller
{
    ...
    public function handleGoogleCallback()
    {
        try {
            ...
            if (!empty($checkuser)) {
                return Redirect::to('/signin')->with('danger', 'Email đã tồn tại');
            }
            ...
            if ($usergoogle != "" OR @$usergoogle->email == $user->email AND $user->email != "") {
                if ($usergoogle->mobile == "") {
                    ...
                    if($usergoogle->is_verified == '1') 
                    {
                        ...
                        return Redirect::to('/email-verify')->with('success', 'Email đã được gửi tới email đăng ký của bạn'); 
                    }
                }
            } else {
            ...
            return Redirect::to('/signin')->with('danger', 'Đã có lỗi xảy ra');
        }
    }
}
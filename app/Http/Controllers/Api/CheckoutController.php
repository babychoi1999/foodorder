...
class CheckoutController extends Controller
{
    	...
    	try {
    	    ...
    	    if($request->payment_type == "1") {
                ...
                if ($request->order_type == "2") {
                    ...
                    if($request->address == ""){
                        return response()->json(["status"=>0,"message"=>"Bạn chưa nhập địa chỉ"],400);
                    }

                    if($request->lat == ""){
                        return response()->json(["status"=>0,"message"=>"Hãy chọn 1 địa chỉ từ gợi ý"],400);
                    }
                    ...
                    if($request->pincode == ""){
                        return response()->json(["status"=>0,"message"=>"Bạn chưa nhập mã vùng"],400);
                    }

                    if($request->building == ""){
                        return response()->json(["status"=>0,"message"=>"Bạn chưa nhập số nhà"],400);
                    }

                    if($request->landmark == ""){
                        return response()->json(["status"=>0,"message"=>"Bạn chưa nhập địa điểm"],400);
                    }
    	    ...
    	    return response()->json(['status'=>0,'message'=>'Đã có lỗi'.$e],400);
    	}
    }
    ...
    public function wallet(Request $request)
    {
        ...
        {
            return response()->json(['status'=>1,'message'=>'Transaction list Successful','walletamount'=>number_format($walletamount->wallet),'data'=>$transaction_data],200);
        }   
        else
        {
            return response()->json(['status'=>0,'message'=>'No data found'],200);

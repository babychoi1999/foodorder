...
class DriverController extends Controller
{
    public function login(Request $request )
    {
        ...
        if($request->password == ""){
            return response()->json(["status"=>0,"message"=>"Bạn chưa nhập mật khẩu"],400);
        }
    ...
    public function changepassword(Request $request)
    {
        ...
        if(Hash::check($request['old_password'],@$check_user->password))
        {
            ...
            return response()->json(['status'=>1,'message'=>'Đã thay đổi mật khẩu'],200);
        }
        else{
            return response()->json(['status'=>0,'message'=>'Mật khẩu chưa chính xác'],400);
        }
    }
    ...
    {
        if($request->email == ""){
            return response()->json(["status"=>0,"message"=>"Bạn chưa nhập email để lấy lại mật khẩu"],400);
        }

...
class ItemController extends Controller
{
        ...
        ->get()->first();
            try {
                ...
                if (@$data->addons_id == $request['addons_id']) {
                  ...
                  return response()->json(['status'=>1,'message'=>'Đã cập nhật số lượng sản phẩm trong giỏ hàng'],200);
                  ...
                  return response()->json(['status'=>1,'message'=>'Đã cập nhật số lượng sản phẩm trong giỏ hàng'],200);
                    ...
                    return response()->json(['status'=>1,'message'=>'Sản phẩm đã được thêm vào giỏ hàng','cartcnt'=>$count],200);
                }
    ...
    public function search(Request $request)
    {
        ...
        $countgetitem = Item::with(['category','itemimage'])->select('item.cat_id','item.id','item.item_name','item.item_price','item.item_description',DB::raw('(case when favorite.item_id is null then 0 else 1 end) as is_favorite'))
        ->leftJoin('favorite', function($query) use($user_id) {
            $query->on('favorite.item_id','=','item.id')
            ->where('favorite.user_id', '=', $user_id);
        })->where('item.item_name','LIKE','%' . $request->item . '%')->where('item.item_status','1')->where('item.is_deleted','2')->orderBy('id', 'DESC')->count();

        $getdata=User::select('currency')->where('type','1')->first();
        return view('front.search', compact('getcategory','getabout','getitem','getdata','countgetitem'));
    }
}

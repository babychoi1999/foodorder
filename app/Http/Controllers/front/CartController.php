...
class CartController extends Controller
{
    ...
    public function applypromocode(Request $request)
    {
        ...
        $checkpromo=Order::select('promocode')->where('promocode',$request->promocode)->where('user_id',$user_id)->count();

        if ($checkpromo > "0" ) {
            return response()->json(['status'=>0,'message'=>'Bạn chỉ có thể sử dụng khuyến mãi này 1 lần'],200);
        } else {
            ...
            if($promocode['offer_code']== $request->promocode) {
                if(!empty($promocode))
                {
                    return response()->json(['status'=>1,'message'=>'Mã giảm giá đã được áp dụng','data'=>$promocode],200);
                }
            } else {
                return response()->json(['status'=>0,'message'=>'Mã giảm giá không đúng'],200);
            }
        }
    }
    ...
    public function removepromocode(Request $request)
    {
        ...
        if(!$remove) {
            return response()->json(['status'=>1,'message'=>'Đã xóa mã giảm giá'],200);
        } else {
            return response()->json(['status'=>0,'message'=>'Đã có lỗi xảy ra.'],200);
        }
    }
    ...
    public function isopenclose(Request $request)
    {
        ...
        if ($current_time > $open_time && $current_time < $close_time && $isopenclose->always_close == "2") {
           return response()->json(['status'=>1,'message'=>'Cửa hàng đang mở cửa.'],200);
        } else {
           return response()->json(['status'=>0,'message'=>'Cửa hàng đã đóng cửa.'],200);
        }
    }
}

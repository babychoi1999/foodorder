...
class OrderController extends Controller
{
    ...
    public function cashondelivery(Request $request)
    {
        if ($request->order_type == "1") {
            if($request->address == ""){
                return response()->json(["status"=>0,"message"=>"Bạn chưa nhập địa chỉ"],200);
            }
            ...
            if($request->postal_code == ""){
                return response()->json(["status"=>0,"message"=>"Bạn chưa nhập mã vùng"],200);
            }
    ...
    public function walletorder(Request $request)
    {
        ...
        if ($getuserdata->wallet < $request->order_total) {
            return response()->json(["status"=>0,"message"=>"Số tiền trong ví của bạn không đủ để thực hiện giao dịch"],200);
        }
        ...
        try {
            ...
            if ($request->order_type == "2") {
                ...
                if(@$pincode['pincode'] == $request->postal_code) {
                    ...
                    return response()->json(['status'=>0,'message'=>'Chúng tôi chỉ giao hàng trong khu vực thành phố Hồ Chí Minh'],200);
                }
            }
